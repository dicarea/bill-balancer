...
public class GesPagosDataSource {
    ...
    public Map getNumTaken(Integer idGroup) {
        Map ret = new HashMap();
        String sql = "select o.idMate, (select sum(i.numItems) from payment i where i.idMate = o.idMate) out " +
                "from payment o group by o.idMate";
    ...
    public Map getNumPayed(Integer idGroup) {
        Map ret = new HashMap();
        String sql = "select o.idMate, (select sum(i.numItems) from payment i where i.idRound in " +
                "(select ii.idRound from payment ii where ii.idMate = o.idMate and ii.isPayer = 1)) out " +
                "from payment o group by o.idMate";
        Cursor cursor = mDatabase.rawQuery(sql, null);
        try {
            cursor.moveToFirst();
