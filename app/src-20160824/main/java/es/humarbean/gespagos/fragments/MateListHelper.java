...
public class MateListHelper {
    ...
    public void markMultiplier(Integer mateId, boolean checked) {
        ds.getMateById(mateId).setmMultiplierActive(checked);
    }
    ...
    public void processPayment() {
        ...
        for (Mate mate : mates) {
            if (mate.isSelected()) {
                int multiplier = mate.ismMultiplierActive() ? mate.getMultiplier() : 1;
                numMatesInRound = numMatesInRound + multiplier;
            }
        }
        ...
        for (Mate mate : mates) {
            if (mate.isSelected()) {
                ...
                mate.setNumRounds(mate.getNumRounds() + multiplier);
                mate.setNumMatesInRounds(mate.getNumMatesInRounds() + numMatesInRound);
                if (mate.isPayer()) {
                    /* This mate is payer. */
                    mate.setNumPayments(mate.getNumPayments() + multiplier);
    ...
    public void insertRoundAndPayments() {
        ...
        for (Mate mate : mates) {
            if (mate.isSelected()) {
                ...
                payment.setNumItems(mate.getMultiplier());
